...
namespace fst
{
    ...
    {
        for(const auto text : parser.textures){
            textures.push_back(Texture(0 , 0 , nullptr , text.imageName , text.interpolation , text.decalMode , text.appearance));
        }
        ...
        for (auto& mesh : parser.meshes)
        {
            ...
            for (auto& face : mesh.faces)
            {
                ...
                if(face.v0_id - 1 < tex_coord_data.size()){
                    triangles.back().u_v_0 = tex_coord_data[face.v0_id - 1];
                    triangles.back().u_v_1 = tex_coord_data[face.v1_id - 1];
                    triangles.back().u_v_2 = tex_coord_data[face.v2_id - 1];
                }
            }

            meshes.push_back(Mesh(std::move(triangles), mesh.material_id , mesh.texture_id , mesh.transformations));
        }
        ...
        for (auto& triangle : parser.triangles)
        {
            ...
            if(triangle.indices.v0_id - 1 < tex_coord_data.size()){
                triangles.back().u_v_0 = tex_coord_data[triangle.indices.v0_id - 1];
                triangles.back().u_v_1 = tex_coord_data[triangle.indices.v1_id - 1];
                triangles.back().u_v_2 = tex_coord_data[triangle.indices.v2_id - 1];
            }

            meshes.push_back(Mesh(std::move(triangles), triangle.material_id , triangle.texture_id , triangle.transformations));
        }
    ...
    bool Scene::intersect(const Ray& ray, HitRecord& hit_record, float max_distance)
    {
        HitRecord temp;
        ...
        for (auto& sphere : spheres)
        {
            if (sphere.intersect(ray, temp, min_distance))
            {
                ...
                hit_record.sphere = &sphere ;
                hit_record.triangle = nullptr ;
        ...
        for (auto& mesh : meshes)
        {
            if (mesh.intersect(ray, temp, min_distance))
            {
                ...
                hit_record.sphere = nullptr ;
                hit_record.mesh = &mesh ;
            }
        }
    ...
    bool Scene::intersectShadowRay(const Ray& ray, float max_distance)
    {
        for (auto& sphere : spheres)
        {
